-- =========================================================================
-- LAB # 3 : Advanced SQL: Aggregations & Analytics
-- =========================================================================

-- =====================================================
-- Query 1: Total Revenue from Delivered Orders
-- =====================================================
...
-- =====================================================
-- Query 3: Revenue Generated by Each Customer
-- =====================================================
...
ORDER BY total_amount DESC ) AS rank_within_customer
FROM orders
...
SELECT month, revenue, 
      LAG(revenue) OVER (ORDER BY month) AS prev_month, revenue - LAG(revenue) OVER (ORDER BY month) AS absolute_change,
      ROUND(100.0 * (revenue - LAG(revenue) OVER (ORDER BY month))/ NULLIF(LAG(revenue) OVER (ORDER BY month), 0),1) AS pct_change
...
WITH customer_spend AS (
  SELECT c.customer_id, c.name, c.city, COALESCE(SUM(o.total_amount), 0) AS total_spent
  FROM customers c
  LEFT JOIN orders o
...
combined AS (
  SELECT c.name,c.city, s.total_sessions, s.total_pages, s.avg_duration, s.converted_sessions,
...
SELECT name, city, total_sessions, total_pages, avg_duration, total_orders, total_spent,
      ROUND(
